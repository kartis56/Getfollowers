...
sub get_followers_list {  # Usage:  get_followers_list($screen_name) ;
    ...
    while ($arg{'cursor'}) { # 一度に5000までしか取得できないのでcursorを書き換えながら取得を繰り返す
        ...
        while (my @ids_100 = splice(@l_ids,0,100)){
          push @l_users, select_users( @ids_100 );
          
          if ( $debug == 1 ) {    print "user Count: ".scalar @l_users . "\n"; }
          
        }
        if ( scalar( @l_users ) >= 1 ) {
          while (my @tmp_user = splice(@l_users,0,100)){
            
            wait_for_rate_limit('users_lookup');
            if ($check_block == 1) {
              wait_for_rate_limit('friend_lookup');
            }
            users_lookup(@tmp_user) ;
...
# ====================
sub select_users {  # usage: @userinfo = select_users(@user_id_list)
    @_ or die "ERROR: select_users() : user_id_list is empty\n" ;
    my @user_id_list = @_ ;
    
    if ($debug == 1) { print " -- select_users call  --\n" ; }
    my $iter ;
    $iter = $teng->search( 'user_ids', +{'id' => {'IN' => \@user_id_list} },  );
    my $rowall = $iter->all;
    ...
    if ( $debug == 1 ) {    print "UserInfo Count : " .scalar @user_info . "\n"; }
    my @diff = grep { ++$cnt{$_} == 1 } @user_id_list;  
    if ( $debug == 1 ) {    print "Diff Count : " .scalar @diff . "\n"; }
   # if ( $debug == 1 ) {    print Dumper @unk_info; }
    $teng->bulk_insert( 'unknown' , \@unk_info ) ;
